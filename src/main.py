...
from src.constants import LAYER_SIZE, BATCH_SIZE
...
class DQN(nn.Module):
    ...
    def __init__(self, inputs, outputs):
        ...
        self.hl1 = nn.Linear(inputs, LAYER_SIZE)
        self.hl2 = nn.Linear(LAYER_SIZE, LAYER_SIZE)
        self.hl3 = nn.Linear(LAYER_SIZE, outputs)
...
def plot_durations(show_result=False):
    plt.figure(1)
    durations_t = torch.tensor(race_times, dtype=torch.float)
    if show_result:
        plt.title("Result")
    else:
        plt.clf()
...
def main():
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            if done:
                race_times.append(env.car.current_time)
                plot_durations()
                break

    print("Complete")
    print("Average Race Time:", format_time(sum(race_times) / len(race_times)))
